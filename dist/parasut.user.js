...
(() => {
  ...
  // src/parasut/transactionHistory.js
  ...
  function findLinkedRecordByText(recordName, options) {
  ...
  function isTransactionHistoryVisible() {
    const root = getActiveAppDocument();
    return Boolean(
      $("[data-test-contact-show-tx-history-row-description]", root) || $("[data-tid='close-tx-history']", root) || $("[class*='show-tx-history']", root)
    );
  }
  function isTransactionHistoryLoaded() {
    const root = getActiveAppDocument();
    return $$("[data-test-contact-show-tx-history-row-description]", root).some(
      isVisible
    ) || Boolean($("[class*='pagination-info']", root)) || Boolean($("[data-tid='export-button']", root));
  }
  function findShowTransactionHistoryButton() {
    const root = getActiveAppDocument();
    const wanted = norm("\u0130\u015ELEM GE\xC7M\u0130\u015E\u0130N\u0130 G\xD6STER");
    return $$("button", root).filter(isVisible).find(
      (button) => button.getAttribute("data-tid") === "show-tx-history" || norm(elementText(button)) === wanted
    ) || null;
  }
  async function ensureTransactionHistoryVisible(clickLink3) {
    if (isTransactionHistoryVisible()) return true;
    const opener = findShowTransactionHistoryButton();
    if (!opener) return false;
    clickLink3(opener);
    await waitFor(() => isTransactionHistoryLoaded(), 12e3);
    await sleep(300);
    return true;
  }
  function transactionHistoryFingerprint() {
    const root = getActiveAppDocument();
    return $$("[data-test-contact-show-tx-history-row-description]", root).slice(0, 3).map((el) => {
  ...
  function findTransactionHistoryPageLink(pageNumber) {
    const expected = String(pageNumber);
    const root = getActiveAppDocument();
    return $$("a[href]", root).filter(isVisible).find((anchor) => {
      const search = hrefSearch(anchor);
      const isPageNumber = norm(elementText(anchor)) === expected;
      const isPaginationLink = Boolean(anchor.closest("[class*='page-numbers']")) || Boolean(anchor.closest("[data-test-page]"));
      return search.includes(`islem-gecmisi-sayfa=${expected}`) || isPageNumber && isPaginationLink;
    }) || null;
  }
  async function goToTransactionHistoryPage(pageNumber, clickLink3) {
    ...
    await waitFor(() => isTransactionHistoryLoaded(), 5e3).catch(() => null);
    await sleep(300);
    return true;
  }
  async function findRecordLinkWithHistory(recordName, options) {
    const findLink = () => findLinkedRecordByText(recordName, options);
    let link = await waitFor(findLink, options.initialTimeout || 9e3).catch(
      () => null
    );
    if (!link) {
      ...
      if (historyOpened) {
        options.onProgress?.(
          options.historyProgress || `\u0130\u015Flem ge\xE7mi\u015Finde aran\u0131yor: ${recordName}`
        );
    ...
    if (!link && isTransactionHistoryVisible()) {
      options.onProgress?.(
        options.secondPageProgress || `\u0130\u015Flem ge\xE7mi\u015Fi 2. sayfa kontrol ediliyor: ${recordName}`
      );
      ...
      if (movedToSecondPage) {
        link = await waitFor(findLink, options.secondPageTimeout || 3e3).catch(
          () => null
        );
      }
    }
  ...
  function findSupplierRowLink(supplierName) {
    ...
    const detailLinks = $$("a[href]", root).filter(
      (a) => /\/tedarikciler\/\d+/.test(hrefPath2(a)) && isVisible(a)
    );
    ...
    const target = findByText2(names, supplierName);
  ...
  async function openExpenseItem(record, onProgress = () => {
    ...
    const link = await findRecordLinkWithHistory(record.itemName, {
      ...
      clickLink,
      onProgress,
      historyProgress: `\u0130\u015Flem ge\xE7mi\u015Finde aran\u0131yor: ${record.itemName}`,
      secondPageProgress: `\u0130\u015Flem ge\xE7mi\u015Fi 2. sayfa kontrol ediliyor: ${record.itemName}`
    });
    if (!link) {
  ...
  function findByText3(elements, wanted) {
    const target = norm(wanted);
    return elements.find((el) => norm(elementText(el)) === target) || elements.find((el) => norm(elementText(el)).includes(target)) || elements.find((el) => target.includes(norm(elementText(el)))) || null;
  }
  function hrefPath3(anchor) {
    return (anchor.getAttribute("href") || "").split("#")[0].split("?")[0];
  }
  function pickAnchorByText3(anchors, wanted, labelSelector) {
    const target = norm(wanted);
    const textOf = (anchor) => {
      const label = labelSelector ? anchor.querySelector(labelSelector) : null;
      return norm(elementText(label || anchor));
  ...
  function findEmployeeRowLink(employeeName) {
    const root = getActiveAppDocument();
    const detailLinks = $$("a[href]", root).filter(
      (a) => /\/calisanlar\/\d+/.test(hrefPath3(a)) && isVisible(a)
    );
    const byHref = pickAnchorByText3(
      detailLinks,
      employeeName,
      "[data-test-display-name]"
    );
    if (byHref) return byHref;
    const names = $$("[data-test-display-name]", root).filter(isVisible);
    const target = findByText3(names, employeeName);
    return target ? target.closest("a[href]") || target.closest("a") : null;
  }
  ...
  async function openSalaryItem(salaryTitle, onProgress = () => {
    ...
    const link = await findRecordLinkWithHistory(salaryTitle, {
      ...
      clickLink: clickLink2,
      onProgress,
      historyProgress: `\u0130\u015Flem ge\xE7mi\u015Finde maa\u015F kayd\u0131 aran\u0131yor: ${salaryTitle}`,
      secondPageProgress: `\u0130\u015Flem ge\xE7mi\u015Fi 2. sayfada maa\u015F kayd\u0131 kontrol ediliyor: ${salaryTitle}`
    });
    if (!link) {
